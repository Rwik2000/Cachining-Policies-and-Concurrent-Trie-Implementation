// A file to check and make csv data for HOH locking
// on the custom data
#include <stdio.h>
#include <stdlib.h>
#include <unistd.h>
...
int main(){
    ...
    // Writer intensive
    FILE *file_ptr_loop = fopen("plot/hoh_lock/writer_intensive.csv","w");
    for(int j=5;j<301;j++){
        trie = init_trie();
        // 80% write(insertion)
        // 20% read (finding)
    ...
    // Reader intensive
    file_ptr_loop = fopen("plot/hoh_lock/reader_intensive.csv","w");
    for(int j=5;j<301;j++){
        trie = init_trie();
        // 20% write(insertion)
        // 80% read (finding)
    ...
    // Equal intensive
    file_ptr_loop = fopen("plot/hoh_lock/equal_intensive.csv","w");
    for(int j=5;j<301;j++){
        trie = init_trie();
        // 50% write(insertion)
        // 50% read (finding)
