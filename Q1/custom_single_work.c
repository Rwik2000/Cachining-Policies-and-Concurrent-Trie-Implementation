// A file to check and make csv data for Single mutex locking
// on the custom data
#define _S_LOCK_TRIE
...
int main(){
    ...
    // Writer intensive
    FILE *file_ptr_loop = fopen("plot/single_lock/writer_intensive.csv","w");
    for(int j=5;j<301;j++){
        trie = init_trie();
        // 80% write(insertion)
        // 20% read (finding)
    ...
    // Reader intensive
    file_ptr_loop = fopen("plot/single_lock/reader_intensive.csv","w");
    for(int j=5;j<301;j++){
        trie = init_trie();
        // 20% write(insertion)
        // 80% read (finding)
    ...
    // Equal intensive
    file_ptr_loop = fopen("plot/single_lock/equal_intensive.csv","w");
    for(int j=5;j<301;j++){
        trie = init_trie();
        // 50% write(insertion)
        // 50% read (finding)
